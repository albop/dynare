...
if max(abs(oo_.SmoothedVariables.Y_obs-log(Y_obs')))>1e-5 ||...
    max(abs(oo_.SmoothedVariables.P_obs-log(P_obs')))>1e-5 || ...
    max(abs(oo_.SmoothedVariables.junk2-log(junk2')))>1e-5
    error('Smoothed Variables are wrong')
end

if max(abs(oo_.UpdatedVariables.Y_obs-log(Y_obs')))>1e-5 ||...
    max(abs(oo_.UpdatedVariables.P_obs-log(P_obs')))>1e-5 || ...
    max(abs(oo_.UpdatedVariables.junk2-log(junk2')))>1e-5
    error('Updated Variables are wrong')
end

if mean(abs(oo_.FilteredVariables.Y_obs(1:end-1)-log(Y_obs(2:end)')))>1e-3 ||...
    mean(abs(oo_.FilteredVariables.P_obs(1:end-1)-log(P_obs(2:end)')))>1e-3 
    error('Smoothed Variables are wrong')
end

if abs(corr(oo_.FilteredVariables.Y_obs(2:end-1)-log(Y_obs(3:end)'),oo_.FilteredVariables.Y_obs(1:end-2)-log(Y_obs(2:end-1)')))>1e-2 ||...
    abs(corr(oo_.FilteredVariables.P_obs(2:end-1)-log(P_obs(3:end)'),oo_.FilteredVariables.P_obs(1:end-2)-log(P_obs(2:end-1)')))>2e-1 ||...
    abs(corr(oo_.FilteredVariables.junk2(2:end-1)-log(junk2(3:end)'),oo_.FilteredVariables.junk2(1:end-2)-log(junk2(2:end-1)')))>1e-2 
