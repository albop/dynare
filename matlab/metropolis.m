...
if options_.forecast | options_.smoother | options_.filtered_vars
    ...
    for j=1:nvobs
		for i=1:nvar
	  		iobs = strmatch(options_.varobs(j,:),lgy_,'exact');
		end
		IdObs(j,1) = iobs; 
    end    
		...
		if nfile-ffil+1>1			
  			...
  			for b = 1:B;
    			...
    			if options_.forecast
    				...
    				dr = resol(ys_,options_.dr_algo,options_.linear,options_.order);
    				...
    				for j = 1:nvar	% The state of the economy at the end of the sample 
						...
						if any(j==IdObs)
			  				idx = find(j==IdObs);
			  				yyyy(dr.order_var(j),1:ykmin_) = data(idx,size(data,2)-ykmin_+1:end);
						else
			  				yyyy(dr.order_var(j),1:ykmin_) = atT(j,size(atT,2)-ykmin_+1:size(atT,2));
						end    
    				end
    				if irun_forc < MAX_nforc
      					stock_forcst(:,:,irun_forc) = transpose(simult_(yyyy,dr,ex_,options_.order));
    				else
      					stock_forcst(:,:,irun_forc) = transpose(simult_(yyyy,dr,ex_,options_.order));
...
if options_.moments_varendo
  if ~isempty(options_.unit_root_vars)
    ...
    ivar = zeros(nvar,1);
    for i = 1:nvar
      ivar(i) = strmatch(varlist(i,:),lgy_,'exact');
    end
  else
    nvar	= size(varlist,1);
    ivar = zeros(nvar,1);
