...
if options_.mode_compute > 0
  ...
  elseif options_.mode_compute == 2
    ...
    if isempty(strmatch('dsge_prior_weight',estim_params_.param_names)) & isempty(dsge_prior_weight)
      [fval,xparam1,grad,hessian_asamin,exitflag] = asamin('minimize','DsgeLikelihood',xparam1,lb,ub,-ones(size(xparam1)),gend,data);   
    else
      [fval,xparam1,grad,hessian_asamin,exitflag] = asamin('minimize','DsgeLikelihood',xparam1,lb,ub,-ones(size(xparam1)),gend);   
    end  
  ...
  elseif options_.mode_compute == 4
    ...
    if isempty(strmatch('dsge_prior_weight',estim_params_.param_names)) & isempty(dsge_prior_weight)
      ...
      [fval,xparam1,grad,hessian_csminwel,itct,fcount,retcodehat] = csminwel('DsgeVarLikelihood',xparam1,H0,[],crit,nit,gend);
      disp(sprintf('Objective function at mode: %f',fval))
